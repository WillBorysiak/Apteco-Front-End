...
- The main feature of the application is a table which houses all of the live data pulled from the Apteco API.
  <br>
- The table can be sorted by its values, filtered based on countries and paginated to get clear and readable content.
...
- The table is generated by pushing data into the Angular Material table component which displays the country, the year and the attendance numbers.
  <br>
- I used a Node Express backend with Axios privately to authenticate my POST request to the Apteco API and return the data to my service.
...
- I would like to make the page more responsive for mobile users as the MatTable doesnt follow the normal flow of the document and can cause horizontal scrolling on smaller devices. I would spend time breaking down how the MatTable CSS's works however that could have been very time consuming.
  <br>
- I would like to see if there was a more efficient way to handle the enviromental variables in Angular as the backend solution took some time and effort to get working. There might be a method that involves the enviroments baked into Angular I didn't discover? I am in-fact very grateful as it allowed me to get a taste of a full stack application for the first time.
  <br>
- Touching on from the previous point, I would like to see how I could build out my Express app to make it more complex or efficient. I barely scraped the surface of what Node can do and whilst it covered my basic needs, it does leave me thinking what else can it do?
  <br>
- I am rather curious to see just how 'easy' this challenge would be using React and see what the experience is like rendering tables of data, I'm sure I'll get this pleasure at some point in the future.

## Reflecting on my Assumptions

Its fair to say the two areas I intitially identified were the exact two areas that presented the biggest challenge to overcome.

- Angular is definitely more difficult to use than React. Understanding how to use the module system and the syntax for certain features took some practise to grasp. Angular uses classes to manage its component logic which is quite different to the functional components of React (however React was initially class based as well). Once I got a grap of how to pass data through components, conditionally render and how observables work, the project suddenly clicked.
  <br>
- Getting the API to work and presenting data was again another another area that demanded my time. The data processing was straight forward once I got the separate values and the format they needed to be supplied in. The API could have been very simple if I hard coded my access token however this isnt good practise at all. I went down the rabbit hole of NodeJS but came out the otherside with a backend server that handled this issue in the proper manner.
